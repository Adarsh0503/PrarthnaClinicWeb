...
import { z } from 'zod'
import toast from 'react-hot-toast'
import { bookingAPI, doctorAPI } from '@/lib/api'
import { CalendarDays, Clock, User, Stethoscope } from 'lucide-react'
...
export default function BookingPage() {
  ...
  useEffect(() => {
    ...
    doctorAPI.getAll().then(res => {
      ...
      setDoctors([
        { id: 1, name: 'Dr. Paritosh Mishra', specialization: 'General Physician' },
        { id: 2, name: 'Dr. Rajni Mishra',    specialization: 'Dentist' },
      ])
    })
  }, [])
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center px-6">
        <div className="bg-white rounded-3xl p-12 max-w-md w-full text-center shadow-xl">
          ...
          <a href="/patient/dashboard" className="btn-primary inline-block">View My Bookings</a>
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      {!isLoggedIn && (
        <div className="max-w-5xl mx-auto px-6 pt-8">
          <div className="bg-yellow-50 border border-yellow-200 rounded-2xl p-4 text-center">
            <p className="text-yellow-800 text-sm font-medium">
              Please <a href="/login" className="underline font-bold">login</a> or{' '}
              <a href="/register" className="underline font-bold">register</a> to book an appointment.
            </p>
          </div>
        </div>
      )}
